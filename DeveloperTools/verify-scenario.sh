...
# Acquired BEFORE the slow device resolve so a competing session cannot pass
# its own guard inside that window. The lock is owned by this shell's PID, so
# it stays valid while an orphaned run keeps working after its conversation
# dies, and goes stale the moment the process actually exits.
set +e
