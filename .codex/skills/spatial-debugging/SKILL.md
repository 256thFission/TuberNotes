...
5. Check the same anchors before and after pan, zoom, rotation, and layout changes. A correct Pin moves with page content and keeps its page-normalized location. Pin the target using `Docs/DeviceWorkflow.md`, run `DeveloperTools/verify-scenario.sh fake-pin` and `multi-pin` for deterministic state, then inspect before/after directly on that iPad once viewport transforms exist.
6. Keep conversion functions small and explicit. Add focused tests only for transforms that can regress independently, then inspect the connected physical iPad. Stop if it is unavailable; never substitute a simulator.
