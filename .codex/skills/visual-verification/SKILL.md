...
5. Ask a human to judge Apple Pencil feel, interaction quality, animation feel, and visual taste on appropriate hardware. When a durable record is needed, use Skill `human-device-loop` (`request_human_review` / `request_pen_fixture`) rather than an ad-hoc chat question.
