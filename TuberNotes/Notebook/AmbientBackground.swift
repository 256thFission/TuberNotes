...
struct AmbientBackground: View {
    ...
    private let blobs: [Blob] = [
        Blob(baseX: 0.20, baseY: 0.18, radius: 320, driftX: 60, driftY: 44, driftSpeed: 0.06, breathSpeed: 0.50, phase: 0.0, alpha: 0.30),
        Blob(baseX: 0.82, baseY: 0.14, radius: 280, driftX: 52, driftY: 56, driftSpeed: 0.05, breathSpeed: 0.42, phase: 1.4, alpha: 0.20),
        Blob(baseX: 0.72, baseY: 0.72, radius: 360, driftX: 70, driftY: 62, driftSpeed: 0.045, breathSpeed: 0.37, phase: 2.6, alpha: 0.26),
        Blob(baseX: 0.24, baseY: 0.82, radius: 300, driftX: 56, driftY: 48, driftSpeed: 0.07, breathSpeed: 0.55, phase: 3.7, alpha: 0.18),
        Blob(baseX: 0.50, baseY: 0.46, radius: 420, driftX: 44, driftY: 70, driftSpeed: 0.038, breathSpeed: 0.31, phase: 5.0, alpha: 0.15),
    ]
            ...
            Canvas { ctx, size in
                ...
                ctx.drawLayer { layer in
                    ...
                    for blob in blobs {
                        ...
                        let breath = 1 + 0.18 * CGFloat(sin(t * blob.breathSpeed + blob.phase))
