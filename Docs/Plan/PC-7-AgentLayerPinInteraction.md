...
- 2026-07-21 — User clarified the Pin/conversation boundary: a Pin keeps its
  initial summary, ordinary messages remain inside that Pin, and only an
  explicit message fork may add another Pin. Implemented additive
  `PinConversationMessage`, `PageAnnotation.conversationMessages`, and
  `PageAnnotation.forkedFromMessageID`; Pin Chat now renders a cycle-safe
  message tree with “Fork from here,” while expanded Pins expose “Open
  conversation.” Ordinary responses mutate only the owning Pin's message list;
  the explicit fork response is the only follow-up branch that appends a new
  annotation. Focused plus nearby checks pass 21/21 under
